...
class PluginLoader:
        ...
        *,
        ...
        if not self._contained_file(root, manifest_path):
            self._diag(
                ...
                "plugin.json is missing or escapes plugin root",
                "blocker",
                "plugin.json",
            )
            return bundle
        ...
        for field in sorted(set(normalized) - _MANIFEST_FIELDS):
            self._diag(
                ...
                f"unknown top-level field ignored: {field}",
                "warning",
                f"plugin.json/{field}",
                "ignored",
            )
        ...
        if errors:
            for error in errors:
                self._diag(
                    ...
                    error.message,
                    "blocker",
                    self._json_path("plugin.json", error.path),
                    "unsafe",
                )
            return bundle
        ...
        bundle.manifest = PluginManifest(
            ...
            keywords=tuple(normalized.get("keywords", [])),
        ...
        for skill_dir in sorted(skills_root.iterdir()):
            ...
            if not self._contained_file(skills_root, skill_file):
                if skill_file.exists() or skill_file.is_symlink():
                    self._diag(
                        bundle,
                        "builtin.skill_path_escape",
                        "builtin SKILL.md escapes its root",
                        "error",
    ...
    def _load_skills(self, bundle: PluginBundle) -> None:
        ...
        if not self._contained_dir(bundle.root, skills_root):
            self._diag(
                ...
                "skills must be a contained directory",
                "error",
                "skills",
                "unsafe",
            )
        ...
        *,
        ...
        try:
            ...
            self._diag(
                ...
                str(exc),
                "error",
                str(skill_file.relative_to(bundle.root)),
                "unsafe",
            )
            return None
        ...
        if not isinstance(metadata, dict):
            self._diag(
                bundle, "skill.frontmatter_invalid", "frontmatter must be a mapping", "error", path
            )
            return None
        name, description = metadata.get("name"), metadata.get("description")
        problems: list[str] = []
        unknown_fields = sorted(set(metadata) - _SKILL_FIELDS)
        if unknown_fields:
            problems.append(
                "unknown frontmatter fields are not portable: " + ", ".join(unknown_fields)
            )
        ...
        elif enforce_directory_name and name != skill_dir.name:
        ...
        if license_name is not None and (
            ...
            problems.append("license must be a non-empty string")
        extra_metadata = metadata.get("metadata")
        if extra_metadata is not None and (
            not isinstance(extra_metadata, dict)
            or any(
                not isinstance(k, str) or not isinstance(v, str) for k, v in extra_metadata.items()
            )
        ):
        ...
        if allowed_tools is not None and (
            ...
            problems.append("allowed-tools must be a non-empty space-separated string")
        if problems:
        ...
        canonical_metadata = extra_metadata or {}
        return Skill(
            name=name,
            ...
            path=skill_dir,
            source="plugin",
            tags=canonical_metadata.get("cool.tags", "").split(),
            tools=allowed_tools.split() if allowed_tools else [],
            version=canonical_metadata.get("cool.version", "1.0"),
            metadata=metadata,
        )
    ...
    def _load_mcp(self, bundle: PluginBundle, data_root: Path) -> None:
        ...
        if not self._contained_file(bundle.root, path):
            self._diag(
                ...
                "mcp.json must be a contained regular file",
                "error",
                "mcp.json",
                "unsafe",
            )
        ...
        top_errors = [
            error
            for error in self._mcp_validator.iter_errors(raw)
            if list(error.path)[:1] != ["mcpServers"]
        ]
        if (
            raw.get("$schema") != MCP_SCHEMA_ID
            or set(raw) != {"$schema", "mcpServers"}
            or not isinstance(raw.get("mcpServers"), dict)
            or top_errors
        ):
            self._diag(
                bundle,
                "mcp.document_invalid",
                "mcp.json has invalid schema or top-level fields",
                "error",
                "mcp.json",
            )
            return
        assert bundle.manifest is not None
        if bundle.manifest.schema.removesuffix("plugin.schema.json") != raw["$schema"].removesuffix(
            "mcp.schema.json"
        ):
            self._diag(
                bundle,
                "mcp.version_mismatch",
                "mcp.json version does not match plugin.json",
                "error",
                "mcp.json",
            )
            return
    ...
    def _portable_mcp(
        self, bundle: PluginBundle, name: str, raw: dict[str, Any], data_root: Path
        ...
        if transport == "stdio":
            ...
            if collides:
                self._diag(
                    bundle,
                    "mcp.env_reserved",
                    "env must not override PLUGIN_ROOT or PLUGIN_DATA under platform name semantics",
                    "error",
                    f"mcp.json/mcpServers/{name}/env",
                    "unsafe",
                )
                return None
            command = raw["command"]
            if command.startswith("./"):
                ...
                if not resolved.is_relative_to(bundle.root) or not resolved.is_file():
                    self._diag(
                        bundle,
                        "mcp.command_escape",
                        "plugin-relative command is missing or escapes plugin root",
                        "error",
                        f"mcp.json/mcpServers/{name}/command",
                        "unsafe",
                    )
                    return None
                ...
                self._diag(
                    bundle,
                    "mcp.command_invalid",
                    "command must be a bare executable name or start with ./",
                    "error",
                    f"mcp.json/mcpServers/{name}/command",
                    "unsafe",
                )
            ...
            env = {
                key: self._expand(value, bundle.root, data_root)
            ...
            if not cwd_path.is_relative_to(allowed_root):
                self._diag(
                    bundle,
                    "mcp.cwd_escape",
                    "cwd escapes its declared PLUGIN_ROOT or PLUGIN_DATA boundary",
                    "error",
                    f"mcp.json/mcpServers/{name}/cwd",
                    "unsafe",
                )
                return None
        ...
        if not self._valid_remote_url(url):
            self._diag(
                bundle,
                "mcp.url_invalid",
                "remote URL must be absolute HTTP(S), omit credentials and fragments, and use HTTPS outside loopback",
                "error",
                f"mcp.json/mcpServers/{name}/url",
                "unsafe",
            )
            return None
        ...
        if not self._valid_headers(headers):
            self._diag(
                bundle,
                "mcp.headers_invalid",
                "header names must be HTTP tokens, values must not contain controls, and names must be unique case-insensitively",
                "error",
                f"mcp.json/mcpServers/{name}/headers",
                "unsafe",
            )
            return None
    ...
    def _load_hooks(self, bundle: PluginBundle) -> None:
        ...
        if not self._contained_file(bundle.root, path):
            self._diag(
                bundle,
                "hooks.invalid_location",
                "hooks.json escapes plugin root",
                "error",
                f"{COOL_NAMESPACE}/hooks/hooks.json",
                "unsafe",
            )
            return
        raw = self._read_json(bundle, path, fatal=False)
        if (
            ...
            or not isinstance(raw.get("hooks"), list)
            or set(raw) != {"version", "hooks"}
        ):
            self._diag(
                bundle,
    ...
    def _parse_hook(
        ...
        if (
            ...
            self._diag(
                bundle, "hook.invalid", "hook requires string id and supported event", "error", path
            )
            return None
        handler = item.get("handler")
        if not isinstance(handler, dict) or handler.get("type") not in {"command", "mcp"}:
            self._diag(
                bundle, "hook.invalid_handler", "handler type must be command or mcp", "error", path
            )
            return None
        if not self._valid_hook_handler(bundle.root, handler):
            self._diag(
                bundle,
                "hook.invalid_handler",
                "handler has an invalid shape or command path",
                "error",
                path,
                "unsafe",
            )
        ...
        if (
            ...
            self._diag(
                bundle,
                "hook.invalid_options",
                "matcher must be an object, order an integer, and concurrency serial or parallel",
                "error",
                path,
            )
            return None
        ...
        if (
            ...
            self._diag(
                bundle,
                "hook.invalid_capabilities",
                "hook declares an unknown capability",
                "error",
                path,
            )
            return None
    ...
    def _diag(
        ...
        path: str = "",
        status: CompatibilityStatus | Literal["unsupported"] = "unsupported",
    ) -> None:
        if status == "unsupported":
            status = "unsafe" if level in {"error", "blocker"} else "ignored"
        bundle.diagnostics.append(
            PluginDiagnostic(code=code, message=message, level=level, status=status, path=path)
        )
